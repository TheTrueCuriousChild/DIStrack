    ...
    for (const item of items) {
      ...
      if (!batchId) {
        const batchResult = await client.query(
          `INSERT INTO batches (drug_id, batch_no, manufacturer, manufacturing_date, expiry_date, qr_code) VALUES ($1, $2, $3, $4, $5, gen_random_uuid()::text) ON CONFLICT (drug_id, batch_no) DO UPDATE SET batch_no = EXCLUDED.batch_no RETURNING id`,
          [
            item.drug_id,
            item.batch_no,
      ...
      await client.query(
        ...
        [
          facilityId,
          batchId,
          item.quantity_received - item.quantity_damaged,
          receipt.id,
          receivedBy,
        ]
      );
